...
func (s *secureSession) runHandshake_xx(ctx context.Context, fallback bool, initialMsg []byte) (err error) {
	...
	if s.initiator {
		...
		if !fallback {
			...
			log.Debug("stage 1 xx_recvHandshakeMessage", "initiator", s.initiator, "msgbuf", msgbuf, "buf len", len(initialMsg))

			if err != nil {
				log.Debug("stage 1 xx_recvHandshakeMessage decode", "initiator", s.initiator, "error", err)
			...
			//log.Debug("stage 1 xx_recvHandshakeMessage", "initiator", s.initiator, "msgbuf", msgbuf, "payload len", len(plaintext))
		...
		if fallback {
			...
			err = proto.Unmarshal(plaintext, nhp)
			if err != nil {
				return fmt.Errorf("stage 2 responder validation fail: cannot unmarshal payload")
			}

			// set remote libp2p public key
			err = s.setRemotePeerInfo(nhp.GetLibp2PKey())
			if err != nil {
				log.Error("stage 2 responder set remote peer info", "err", err)
				return fmt.Errorf("stage 2 responder read remote libp2p key fail")
			}
	...
	log.Debug("xx_handshake done", "initiator", s.initiator)
	return nil
}
